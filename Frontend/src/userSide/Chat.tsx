...
const RightSidebar = () => {
      ...
      <div className="h-full flex flex-col">
        ...
        <div className="py-2 px-3 bg-white flex flex-row justify-between items-center">
          <div className="flex items-center">
            <div>
              <img className="w-10 h-10 rounded-full" src="https://darrenjameseeley.files.wordpress.com/2014/09/expendables3.jpeg" alt="Group Avatar" />
            </div>
            <div className="ml-4">
        ...
        <div className="flex-1 overflow-auto bg-gray-100">
          <div className="py-2 px-3">
            ...
            <MessageItem name="Sylverter Stallone" message="Hi everyone! Glad you could join! I am making a new movie." time="12:45 pm" color="teal" />
            <MessageItem name="Tom Cruise" message="Hi all! I have one question for the movie" time="12:45 pm" color="purple" />
            <MessageItem name="Harrison Ford" message="Again?" time="12:45 pm" color="orange" />
            <MessageItem name="Russell Crowe" message="Is Andrés coming for this one?" time="12:45 pm" color="orange" />
            <MessageItem name="Sylverter Stallone" message="He is. Just invited him to join." time="12:45 pm" color="teal" />
            <MessageItem name="Andrés" message="Hi guys." time="12:45 pm" isCurrentUser />
            <MessageItem name="Andrés" message="Count me in" time="12:45 pm" isCurrentUser />
            <MessageItem name="Tom Cruise" message="Get Andrés on this movie ASAP!" time="12:45 pm" color="purple" />
          </div>
        </div>
  ...
  const MessageItem = ({ name, message, time, color, isCurrentUser }) => {
    const bgColor = isCurrentUser ? '#E2F7CB' : '#F2F2F2';
    const textColor = color ? `text-${color}` : 'text-grey-dark';
  
    return (
      <div className={`flex ${isCurrentUser ? 'justify-end' : ''} mb-2`}>
        <div className="rounded py-2 px-3" style={{ backgroundColor: bgColor }}>
          {!isCurrentUser && <p className={`text-sm ${textColor}`}>{name}</p>}
